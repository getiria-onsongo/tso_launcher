...
LOAD DATA LOCAL INFILE 'sample_bwa_no_dup' INTO TABLE cnv_sample_name_bwa_pileup_no_dup FIELDS TERMINATED BY '\t';
...
LOAD DATA LOCAL INFILE 'sample_bwa' INTO TABLE cnv_sample_name_bwa_pileup FIELDS TERMINATED BY '\t';
...
LOAD DATA LOCAL INFILE 'sample_bowtie' INTO TABLE cnv_sample_name_bowtie_pileup FIELDS TERMINATED BY '\t';
...
USING(chr,pos);
CREATE INDEX cnv_sample_name_exon_pileup_1 ON cnv_sample_name_exon_pileup(exon_contig_id);
CREATE INDEX cnv_sample_name_exon_pileup_2 ON cnv_sample_name_exon_pileup(chr,pos);
...
USING(chr,pos);
CREATE INDEX cnv_sample_name_exon_bowtie_1 ON cnv_sample_name_exon_bowtie(exon_contig_id);
CREATE INDEX cnv_sample_name_exon_bowtie_2 ON cnv_sample_name_exon_bowtie(chr,pos);

DROP TABLE IF EXISTS cnv_sample_name_exon_bwa_bowtie_ratio;
CREATE TABLE cnv_sample_name_exon_bwa_bowtie_ratio AS
SELECT DISTINCT A.chr, A.pos, (A.coverage/B.coverage) AS bowtie_bwa_ratio
FROM
cnv_sample_name_exon_bowtie A
JOIN
cnv_sample_name_bwa_pileup B
ON(A.chr = B.chr AND A.pos = B.pos);
CREATE INDEX cnv_sample_name_exon_bwa_bowtie_ratio_1 ON cnv_sample_name_exon_bwa_bowtie_ratio(chr,pos);
